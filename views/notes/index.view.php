...
<div class="columns-2 md:columns-3 lg:columns-4">
    <?php foreach ($notes as $note) : ?>
        ...
        <div class="w-full h-min max-h-96 flex flex-col justify-between bg-blue-300 rounded-lg py-4 px-4 hover:bg-blue-400 hover:transform hover:scale-[1.02] transition-transform">
            <div>
                ...
                <p 
                    class="text-gray-800 text-sm text-ellipsis overflow-hidden"
                    style="display: -webkit-box; -webkit-box-orient: vertical; -webkit-line-clamp: 12"
                    ><?= htmlspecialchars($note['body']) ?></p>
            </div>
           
            
                <div class="flex items-center justify-between text-gray-800 mt-3 opacity-0 hover:opacity-100 transition-transform">
                    <p class="text-xs"><?= htmlspecialchars($note['date']) ?></p>
                    <a href="note/edit?=<?= $note['id'] ?>">
                        <button 
                            type="button"
                            class="modal-show
                                    w-8 h-8 rounded-full bg-gray-800 text-white flex items-center justify-center focus:outline-none focus:ring-2 focus:ring-offset-2  focus:ring-black" 
